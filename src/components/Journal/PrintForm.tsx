...
export const PrintForm = forwardRef<HTMLDivElement,Props>(({journal,subjectName},props) => {
    ...
    const numberOfJournalPages = !!journal?.columns.length && Array.from(Array(Math.round(journal?.columns.length/20)).keys());
    const limitLessons = Array.from(Array(18).keys());
    const numberOfLessonsPages = !!journal?.columns.length && Array.from(Array(Math.round(journal?.columns.length/18)).keys());

    console.log(limitColumns);
    return <div className="printForm_container" ref={props}>
                ...
                <section className='printFormJournal__container'>
                    <div className='printFormJournal_top'>
                        ...
                        <div className='printFormJournal_top_dates'>
                            {limitColumns.map(columnNumber => !!journal?.columns[columnNumber + columnNumber*pageNumber] &&
                                <div className="printFormJournal_top_dates_date" key={journal.columns[columnNumber].column_id}>
                                    <p>{journal.columns[columnNumber + columnNumber*pageNumber].date.includes('\n') ? (journal.columns[columnNumber + columnNumber*pageNumber].date.split('\n')[1].split('.')[0]+`/`+journal.columns[columnNumber + columnNumber*pageNumber].date.split('\n')[1].split('.')[1]) : journal.columns[columnNumber + columnNumber*pageNumber].date}</p>
                                </div>
                            )}
        ...
        {!!numberOfLessonsPages && numberOfLessonsPages.map(pageNumber =>
            <div className="printForm_page" key={pageNumber} style={{'height':'1000px'}}>
                <div>
                    <h1 className="printForm_header">ДЕРЖАВНИЙ УНІВЕРСИТЕТ ІНТЕЛЕКТУАЛЬНИХ ТЕХНОЛОГІЙ І</h1>
                    <h1 className="printForm_header">ЗВ’ЯЗКУ</h1>
                    <h1 className="printForm_subheader">ВІДОКРЕМЛЕНИЙ СТРУКТУРНИЙ  ПІДРОЗДІЛ</h1>
                ...
                <div className="printFormLessons_main">
                    ...
                    <div className="printFormLessons_lessons">
                        {limitColumns.map(columnNumber => !!journal?.columns[columnNumber + columnNumber*pageNumber] &&
                            <div key={journal?.columns[columnNumber + columnNumber*pageNumber].column_id} className="printFormLessons_lesson">
                                <div className="printFormLessons_top_type">{journal?.columns[columnNumber + columnNumber*pageNumber].lesson_type}</div>
                                <div className="printFormLessons_top_date">{journal?.columns[columnNumber + columnNumber*pageNumber].date}</div>
                                <div className="printFormLessons_top_theme">{journal?.columns[columnNumber + columnNumber*pageNumber].lesson_topic}</div>
                                <div className="printFormLessons_top_task">Завдання</div>
                            </div>)}
                        {/* {journal?.columns.map(column => 
