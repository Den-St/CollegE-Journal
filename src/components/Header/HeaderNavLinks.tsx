...
export const HeaderNavLinks:React.FC<Props> = ({linksClassName,onGoToSection,}) => {
    ...
    const securityLevelToLinks:Record<number,JSX.Element> = {
        ...
        4:<>
            <Link  to={routes.groups} className={linksClassName + isActiveLink(routes.groups) + isActiveLink(routes.pickJournalSubject) + isActiveLink(routes.journal)}>
                Журнал
                <svg className="underline_mButton headerSvg" xmlns="http://www.w3.org/2000/svg" width="52" height="2" viewBox="0 0 52 2" fill="none">
                    <path d="M1 1H51" strokeLinecap="round"/>
                </svg>
        ...
        5:<>
            <Link  to={routes.adminPanel + `?section=schedule`} className={linksClassName + isActiveLink(routes.adminPanel) + isActiveLink(routes.editGroup) + isActiveLink(routes.createTeacher)}>Адмін-панель
                <svg className="underline_mButton headerSvg" xmlns="http://www.w3.org/2000/svg" width="52" height="2" viewBox="0 0 52 2" fill="none">
                    <path d="M1 1H51" strokeLinecap="round"/>
                </svg>
            </Link>
